...
fn black_tea(s: String, dictionary: Vec<String>) {
    ...
    for word in dictionary {
        ...
        if word.contains(s.as_str().trim()) {
...
fn create_a_word(s: String, dictionary: Vec<String>) {
    ...
    println!("{}", possible_words[random_word]);
}
...
fn parse_dictionary() -> Vec<String> {
    ...
    contents.split_whitespace().filter(|&s| s.chars().all(|c| c.is_ascii_alphabetic())).map(|s| s.to_string()).collect::<Vec<String>>()
}